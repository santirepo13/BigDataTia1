...
SELECT 	ope.id_registro,
		...
		r.nombre as region,
		ope.validacion,
		ope.causa_modificacion
FROM operaciones ope
JOIN departamentos dep on dep.id_departamento = ope.id_departamento
JOIN municipios    mun on mun.id_municipio    = ope.id_municipio AND mun.id_departamento = ope.id_departamento
JOIN productos     pro on pro.id_producto     = ope.id_producto
JOIN regiones      r  on r.id_region         = ope.id_region
